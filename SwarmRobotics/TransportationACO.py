...
class TransportationACO:
    ...
    def __init__(self):
        self.survivors_transported = 0

    def aco_transport(self, robot, survivor, pheromone_map, obstacles):
        ...
        if self.SAFE_ZONE_RECT.collidepoint(rx, ry):
            return True
        ...
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                ...
                distance_to_safezone = math.sqrt((x_position - self.SAFE_ZONE_X) ** 2 + (y_position - self.SAFE_ZONE_Y) ** 2)

                distance_before = math.sqrt((rx - self.SAFE_ZONE_X) ** 2 + (ry - self.SAFE_ZONE_Y) ** 2)
                distance_after = math.sqrt((x_position - self.SAFE_ZONE_X) ** 2 + (y_position - self.SAFE_ZONE_Y) ** 2)
